# Blog Audit Report

**File:** blogs/old-agency-incoming/What Makes a Real Estate Company in Noida Trustworthy for Homebuyers - Blog 3 (1).txt
**Score:** 64/100
**Publishable:** No
**Word Count:** 1858
**Paragraphs:** 5
**Total Issues:** 8

## Publication Gates

| Gate | Status | Details |
|------|--------|---------|
| 1. Factual Accuracy | PASS | No factual issues |
| 2. RERA & Legal Compliance | FAIL | 1 compliance issues |
| 3. Technical SEO Eligibility | FAIL | 1 SEO issues |
| 4. Deployment Integrity | WARN | Check post-deployment |

## HIGH Issues (3)

### CG-META-002: No H1 heading found

- **Owner:** ROI
- **Category:** heading_structure
- **Action:** Add exactly one H1 heading as the main title
- **Acceptance test:** Exactly one H1 present
- **Google rule:** CONTENT-HEADING-001

### CG-CONTENT-004: RERA mentioned but no registration number provided

- **Owner:** ROI
- **Category:** rera_compliance
- **Action:** Add the actual RERA registration number with portal verification link
- **Acceptance test:** RERA registration number is visible and verifiable
- **Google rule:** CONTENT-RERA-001

### CG-CONTENT-006: No internal or external links found in content

- **Owner:** ROI
- **Category:** internal_links
- **Action:** Add 3-5 internal links and 1-3 external authority links
- **Acceptance test:** At least 3 internal links and 1 external authority link present

## MEDIUM Issues (4)

### CG-META-001: Meta description too long (190 chars)

- **Owner:** ROI
- **Category:** meta_description
- **Found:** "Learn what makes a real estate company in Noida trustworthy. Explore key factors like RERA approval, transparency, timely delivery, and premium residential projects for confident homebuying."
- **Action:** Shorten to 120-160 characters
- **Acceptance test:** Description is 120-160 characters

### CG-CONTENT-003: Implied appreciation: Implies price appreciation without evidence

- **Owner:** ROI
- **Category:** unsupported_claim
- **Paragraph:** 5
- **Found:** "Ask them what common areas are maintained? Did they get possession on time? Were there any hidden costs?
Remember that the best builder in Noida will never flinch when you ask to see their past projec"
- **Action:** Remove forward-looking price claim or add 'past performance does not indicate future results'
- **Acceptance test:** No implied price appreciation without disclaimer
- **Editorial rule:** EDITORIAL_INVESTMENT_001

### CG-CONTENT-005: Keyword stuffing: 'real estate' appears 19 times

- **Owner:** ROI
- **Category:** spam_signal
- **Action:** Reduce repetition. Use synonyms and natural phrasing.
- **Acceptance test:** No phrase repeated more than 7 times in one article
- **Google rule:** SPAM-KEYWORD-001

### CG-CONTENT-007: No images found in content

- **Owner:** ROI
- **Category:** image_seo
- **Action:** Add 1 hero image and 2-4 supporting visuals with descriptive alt text
- **Acceptance test:** Hero image + 2 supporting images with specific alt text

## INFO Issues (1)

### CG-PROJECT-008: Reviewer note for Clove County: Publishing a Clove County carpet or super area figure; none is registered yet

- **Owner:** ROI
- **Category:** missing_evidence
- **Action:** Confirm the content does not do this
- **Acceptance test:** Publishing a Clove County carpet or super area figure; none is registered yet
- **Editorial rule:** PROJECT_SPECIFIC_RULE

## Issues by Owner

| Owner | Critical | High | Medium | Low | Total |
|-------|----------|------|--------|-----|-------|
| ROI | 0 | 3 | 4 | 0 | 8 |
